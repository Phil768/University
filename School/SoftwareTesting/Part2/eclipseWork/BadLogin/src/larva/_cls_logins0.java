...
try{
RunningClock.start();
pw = new PrintWriter("C:\\Users\\phili\\OneDrive\\Desktop\\University\\School\\SoftwareTesting\\Part2\\eclipseWork\\BadLogin/src/output_logins.txt");
...
int _state_id_badLoginsProperty = 5;
...
if (0==1){}
else if (_state_id_badLoginsProperty==5){
		if (1==0){}
		else if ((_occurredEvent(_event,8/*goodLogin*/))){
		badLogins =0 ;
_cls_logins0.pw .println ("Good login observed. Bad logins: "+badLogins );

		_state_id_badLoginsProperty = 5;//moving to state unblocked
		_goto_badLoginsProperty(_info);
		}
		else if ((_occurredEvent(_event,6/*badLogin*/)) && (badLogins <2 )){
		badLogins ++;
...
_cls_logins0.pw .println ("Bad login observed. Bad logins: "+badLogins );
		...
		_state_id_badLoginsProperty = 4;//moving to state blocked
		_goto_badLoginsProperty(_info);
		}
}
else if (_state_id_badLoginsProperty==4){
		if (1==0){}
		else if ((_occurredEvent(_event,10/*unlockAccount*/)) && (locked ==false &&lockedTime .compareTo (10 )<0 )){
		badLogins =0 ;
_cls_logins0.pw .println ("Unblocked prematurely! Entering bad state!");

		_state_id_badLoginsProperty = 3;//moving to state unlockedPrematurely
		_goto_badLoginsProperty(_info);
		}
		else if ((_occurredEvent(_event,10/*unlockAccount*/)) && (locked ==false &&lockedTime .compareTo (10 )>=0 )){
		badLogins =0 ;
...
switch(_state_id){
case 5: if (_mode == 0) return "unblocked"; else return "unblocked";
case 4: if (_mode == 0) return "blocked"; else return "blocked";
case 3: if (_mode == 0) return "unlockedPrematurely"; else return "!!!SYSTEM REACHED BAD STATE!!! unlockedPrematurely "+new _BadStateExceptionlogins().toString()+" ";
