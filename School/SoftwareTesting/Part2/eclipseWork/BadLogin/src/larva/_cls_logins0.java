...
if (0==1){}
		...
		if (1==0){}
		else if ((_occurredEvent(_event,22/*goodLogin*/))){
		badLogins =0 ;
_cls_logins0.pw .println ("Good login observed. Bad logins: "+badLogins );

		_state_id_badLoginsProperty = 9;//moving to state unblocked
		_goto_badLoginsProperty(_info);
		}
		else if ((_occurredEvent(_event,20/*badLogin*/)) && (badLogins <2 )){
		badLogins ++;
...
_cls_logins0.pw .println ("Bad login observed. Bad logins: "+badLogins );
		...
		_state_id_badLoginsProperty = 8;//moving to state blocked
		_goto_badLoginsProperty(_info);
		}
}
else if (_state_id_badLoginsProperty==8){
		if (1==0){}
		else if ((_occurredEvent(_event,24/*unlockAccount*/)) && (locked ==false &&lockedTime .compareTo (10 )<0 )){
		badLogins =0 ;
_cls_logins0.pw .println ("Unblocked prematurely! Entering bad state!");

		_state_id_badLoginsProperty = 7;//moving to state unlockedPrematurely
		_goto_badLoginsProperty(_info);
		}
		else if ((_occurredEvent(_event,24/*unlockAccount*/)) && (locked ==false &&lockedTime .compareTo (10 )>=0 )){
		badLogins =0 ;
...
switch(_state_id){
case 9: if (_mode == 0) return "unblocked"; else return "unblocked";
case 8: if (_mode == 0) return "blocked"; else return "blocked";
case 7: if (_mode == 0) return "unlockedPrematurely"; else return "!!!SYSTEM REACHED BAD STATE!!! unlockedPrematurely "+new _BadStateExceptionlogins().toString()+" ";
