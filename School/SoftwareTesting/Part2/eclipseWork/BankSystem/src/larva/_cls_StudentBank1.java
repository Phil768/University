...
if (0==1){}
		...
		if (1==0){}
		else if ((_occurredEvent(_event,32/*deleteAccount*/)) && (noOfAccounts ==0 )){
		_cls_StudentBank0.pw .println ("BAD State!!!");

		_state_id_Accounts = 14;//moving to state badDelete
		_goto_Accounts(_info);
		}
		else if ((_occurredEvent(_event,30/*addAccount*/)) && (noOfAccounts ==5 )){
		noOfAccounts ++;

		_state_id_Accounts = 13;//moving to state tooMany
		_goto_Accounts(_info);
		}
		else if ((_occurredEvent(_event,30/*addAccount*/))){
		noOfAccounts ++;

		_state_id_Accounts = 15;//moving to state start
		_goto_Accounts(_info);
		}
		else if ((_occurredEvent(_event,32/*deleteAccount*/))){
		noOfAccounts --;
...
_cls_StudentBank0.pw .println ("Too many accounts already");
		...
		else if ((_occurredEvent(_event,32/*deleteAccount*/)) && (noOfAccounts >6 )){
		noOfAccounts --;
...
case 13: if (_mode == 0) return "tooMany"; else return "!!!SYSTEM REACHED BAD STATE!!! tooMany "+new _BadStateExceptionStudentBank().toString()+" ";
case 14: if (_mode == 0) return "badDelete"; else return "!!!SYSTEM REACHED BAD STATE!!! badDelete "+new _BadStateExceptionStudentBank().toString()+" ";
