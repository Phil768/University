...
try{
RunningClock.start();
pw = new PrintWriter("C:\\Users\\phili\\workspace\\BankSystem/src/output_StudentBank.txt");
...
if (0==1){}
else if (_state_id_Users==14){
		if (1==0){}
		else if ((_occurredEvent(_event,26/*deleteUser*/)) && (noOfUsers ==0 )){
		_cls_StudentBank0.pw .println ("BAD State!!!");

		_state_id_Users = 13;//moving to state badDelete
		_goto_Users(_info);
		}
		else if ((_occurredEvent(_event,24/*addUser*/)) && (noOfUsers ==5 )){
		noOfUsers ++;

		_state_id_Users = 12;//moving to state tooMany
		_goto_Users(_info);
		}
		else if ((_occurredEvent(_event,24/*addUser*/))){
		noOfUsers ++;

		_state_id_Users = 14;//moving to state start
		_goto_Users(_info);
		}
		else if ((_occurredEvent(_event,26/*deleteUser*/))){
		noOfUsers --;
...
_cls_StudentBank0.pw .println ("Too many users already");
		...
		else if ((_occurredEvent(_event,26/*deleteUser*/)) && (noOfUsers >6 )){
		noOfUsers --;
...
switch(_state_id){
case 14: if (_mode == 0) return "start"; else return "start";
case 12: if (_mode == 0) return "tooMany"; else return "!!!SYSTEM REACHED BAD STATE!!! tooMany "+new _BadStateExceptionStudentBank().toString()+" ";
case 13: if (_mode == 0) return "badDelete"; else return "!!!SYSTEM REACHED BAD STATE!!! badDelete "+new _BadStateExceptionStudentBank().toString()+" ";
