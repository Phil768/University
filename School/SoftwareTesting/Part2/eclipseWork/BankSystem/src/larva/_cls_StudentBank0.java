...
try{
RunningClock.start();
pw = new PrintWriter("C:\\Users\\phili\\Desktop\\University\\School\\SoftwareTesting\\Part2\\eclipseWork\\BankSystem/src/output_StudentBank.txt");
...
if (0==1){}
		...
		if (1==0){}
		else if ((_occurredEvent(_event,8/*deleteUser*/)) && (noOfUsers ==0 )){
		_cls_StudentBank0.pw .println ("BAD State!!!");

		_state_id_Users = 4;//moving to state badDelete
		_goto_Users(_info);
		}
		else if ((_occurredEvent(_event,6/*addUser*/)) && (noOfUsers ==5 )){
		noOfUsers ++;

		_state_id_Users = 3;//moving to state tooMany
		_goto_Users(_info);
		}
		else if ((_occurredEvent(_event,6/*addUser*/))){
		noOfUsers ++;

		_state_id_Users = 5;//moving to state start
		_goto_Users(_info);
		}
		else if ((_occurredEvent(_event,8/*deleteUser*/))){
		noOfUsers --;
...
_cls_StudentBank0.pw .println ("Too many users already");
		...
		else if ((_occurredEvent(_event,8/*deleteUser*/)) && (noOfUsers >6 )){
		noOfUsers --;
