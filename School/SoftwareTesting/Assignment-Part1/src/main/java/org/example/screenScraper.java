...
public class screenScraper {
    ...
    public screenScraper(WebDriver driver) {
        ...
        if(driverStatusProvider != null) {
            //Storing the status of the driver in an integer variable, according to the mock.
            int status = driverStatusProvider.getDriverStatusProvider();

            //If the integer matches the good status.
            if (status == driverStatusProvider.goodDriver) {
                this.driver = driver;
                //If the integer matches the bad status.
